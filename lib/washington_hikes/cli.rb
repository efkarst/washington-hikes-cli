class WashingtonHikes::CLI
  ...
  def list_regions(regions)
    regions.each.with_index(1) do |region,i| 
      puts "#{i}. #{region.name}"
      puts "   Common Features: #{region.common_landscape_features.join(", ")}"
  ...
  def list_hikes(hikes)
    hikes.each.with_index(1) {|hike,i| puts "#{i}. #{hike.name} -- #{hike.length} miles, #{hike.type} -- #{hike.elevation_gain} ft gain"}
  end
